...
describe('Tier One: Projects', () => {
  ...
  describe('Sequelize Model', () => {
    ...
    it('*** deadline must be a valid date', async() => {
      ...
      const deadlineNotInDateFormat = Project.build({ deadline: 'something wrong' });
      try {
        await deadlineNotInDateFormat.validate();
        throw Error('Validation isDate on deadline failed');
      } catch (err) {
        expect(err.message).to.contain('Validation isDate on deadline failed');
      }
    });
